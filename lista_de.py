...
class Lista:
    ...
    def __get_nodo(self, posicion):
        if (posicion < 0) or (posicion >= len(self)):
            ...
            if posicion <= len(self)/2:
                ...
                for i in range(0, len(self)-posicion-1):
    ...
    def insert(self, elemento, posicion=None):
        ...
        if posicion == len(self):
            ...
            aux.anterior = self.__ultimo__
            self.__ultimo__ = aux
        elif posicion == 0:
            self.frente.siguiente.anterior = aux
            aux.siguiente = self.frente.siguiente
            self.frente.siguiente = aux
            aux.anterior = self.frente
        else:
    ...
    def suprime(self, posicion):
        ...
        if (posicion >= 0) and (posicion < len(self)):
            if (posicion + 1) == len(self):
                anterior = self.__ultimo__.anterior
